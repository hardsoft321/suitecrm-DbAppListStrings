	...
	function display()
 	{
        ...
        $ddNodes = $dd->getNodes();
        $dbRes = $GLOBALS['db']->query("SELECT uniq_name FROM dbapplists WHERE deleted = 0");
        while($row = $GLOBALS['db']->fetchByAssoc($dbRes, false)) {
            unset($ddNodes[$row['uniq_name']]);
        }
        ...
        $smarty->assign('dropdowns', $ddNodes); // <- HS321 DbAppListStrings
